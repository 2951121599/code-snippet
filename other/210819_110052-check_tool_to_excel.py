...
def calc_qc_rt_rsd(df):
    ...
    for feature in feature_list:
        ...
        res_rsd = pd.concat([res_rsd, each_rsd], axis=0)
    result = pd.concat([df_qc_rt.T, res_rsd], axis=1)
    result['check_result'] = result['QC_RT_RSD'].apply(lambda x: 1 if x <= 0.8 else 0)  # 根据条件添加一列
    result = result.sort_values(by=['QC_RT_RSD'], ascending=False, na_position='first')  # 降序
    return result
